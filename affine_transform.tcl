...
proc integralSH7 { begin end nosteps func } {
    expr {
	  delta = ($end-$begin)/double($nosteps);
	  hdelta = $delta/2.0;
	  result = 0.0;
...
puts SH6:[timerate {integralSH6 -2 2 10 f }]
puts length:[(L = [string length [info body integralSH6]],  "=", $L/double($LO)*100-100)]%
puts SH7:[timerate {integralSH7 -2 2 10 f }]
puts length:[(L = [string length [info body integralSH7]],  "=", $L/double($LO)*100-100)]%
